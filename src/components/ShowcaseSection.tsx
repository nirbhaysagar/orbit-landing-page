...
const ShowcaseSection = () => {
  ...
  return (
    <section id="showcase" className="bg-transparent py-10 md:py-16 relative overflow-hidden font-inconsolata mix-blend-difference">
      ...
      <div className="mx-auto px-6 md:px-[100px] relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 md:mb-15 gap-7">
          <div>
            <span className="text-[9px] uppercase tracking-[0.4em] text-[#C084FC] mb-3.5 block font-bold">SHOWCASE // 003</span>
            <h2 className="text-3xl md:text-[45px] font-black tracking-tighter uppercase text-white leading-[0.9] font-inter">
        ...
        {/* Carousel Container */}
        <div className="relative w-full h-[350px] md:h-[450px] flex items-center justify-center mt-6 md:mt-10">
          {designs.map((design, index) => {
            ...
            const currentCardW = isMobile ? 220 : cardW;
            const currentCardH = isMobile ? 280 : cardH;
            
            // Tighter overlap on mobile
            const getResponsiveTX = (off: number) => {
              if (off === 0) return 0;
              const abs = Math.abs(off);
              const dir = off > 0 ? 1 : -1;
              const mobileJumps = [0, 45, 80, 110, 130];
              const desktopJumps = [0, 65, 115, 155, 185];
              const jumps = isMobile ? mobileJumps : desktopJumps;
              const percent = jumps[abs] || (jumps[jumps.length - 1] + (abs - (jumps.length - 1)) * 20);
              return dir * percent;
            };

            const tx = getResponsiveTX(offset);
            let scale = getScale(offset);
            let zIndex = 50 - absOffset;
            const opacity = absOffset > (isMobile ? 2 : 3) ? 0 : 1;
              ...
              >
                ...
                >
                  <img
                    src={design.image}
                    alt={design.title}
                    className={`absolute inset-0 w-full h-full transition-all duration-500 ${isHovered && !isMobile ? 'object-contain bg-black/90 scale-100 opacity-100' : (isActive ? 'object-cover opacity-100 scale-105' : 'object-cover opacity-60 scale-100')}`}
                  />
                  ...
                  <div className={`absolute bottom-0 left-0 right-0 p-4 md:p-5 font-inconsolata transition-opacity duration-500 ${isActive || isHovered ? 'opacity-100' : 'opacity-0'}`}>
                    <h4 className="text-white text-[10px] md:text-[12px] font-bold uppercase tracking-widest">{design.title}</h4>
                    <p className="text-white/40 text-[8px] md:text-[9px] uppercase tracking-[0.3em] mt-1">{design.subtitle}</p>
                  </div>

                  {isActive && (
                    <div className="absolute top-4 right-4 w-1.5 h-1.5 md:w-2 md:h-2 rounded-full bg-gradient-to-r from-[#C084FC] to-[#F472B6] shadow-[0_0_10px_rgba(192,132,252,0.8)]" />
                  )}
                </div>
              </div>
            );
        ...
        {/* Navigation Controls */}
        <div className="flex justify-center items-center gap-4 md:gap-6 mt-10 md:mt-12 z-20 relative">
          <button 
            onClick={prevCard} 
          ...
          <div className="flex gap-1.5 md:gap-2">
            {designs.map((_, i) => (
              <button 
                ...
                className={`h-1.5 rounded-full transition-all duration-500 ${i === activeIndex ? 'bg-gradient-to-r from-[#C084FC] to-[#F472B6] w-5 md:w-6' : 'bg-white/20 w-1.5 hover:bg-white/40'}`} 
              />
            ))}
          </div>
